...
**Codex Agent SDK for Go** - Unofficial Go SDK for Codex CLI integration. Provides programmatic interaction through `Query()` (one-shot) and `Client` (streaming) APIs with 100% Python SDK parity.

- **Module**: `github.com/tea4go/Codex-agent-sdk-go`
- **Package**: `Codex`
- **Go Version**: 1.18+
...
1. `Query()`/`Client` -> `Transport` interface -> `subprocess.Transport` -> Codex CLI
